    ...
    with open(chemin_fichier+"/"+str(departement)+"/Master.kml", "w", encoding="cp1252") as p:
        ...
        for ville in villes_box:
            ...
            for nom_fichiers in file_name:
                if os.path.exists("./modifie"+"/"+str(departement)+"/"+nom_fichiers+"/"+nom_fichiers+"_"+ville+".kml"):
                    liste_fichier[nom_fichiers]=nom_fichiers+"/"+nom_fichiers+"_"+ville+".kml"
            p.writelines("\t<Folder>\n\t\t<name>"+ville+"</name>\n")
            for fichier in liste_fichier:
                i+=1
                lien ="https://raw.githubusercontent.com/MaskassIV/"+str(departement)+"/refs/heads/master/"+liste_fichier[fichier].replace(" ", "%20")
