...
def regrouper(chemin_fichier, option_nom_fichier, departement):
    ...
    for nom_fichier in file_name:
        ...
        for ligne in lignes:
            if "<Placemark" not in ligne:
                ...
                break
        lignes_regroupees.append("<Placemark id=\"parcelle_"+str(departement)+"_com_grouped\">\n\t<styleUrl>#"+nom_fichier+"</styleUrl>\n\t<ExtendedData>\n\t\t<SchemaData schemaUrl=\"#parcelle_"+str(departement)+"_com\">\n\t\t\t<SimpleData name=\"Groupe personne\">"+nom_fichier +"</SimpleData>\n\t\t</SchemaData>\n\t</ExtendedData>\n\t<MultiGeometry>\n")
        for ligne in lignes:
